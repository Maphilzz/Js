...
const expirationDatePattern = {
  ...
  blocks: {
    YY: {
      ...
      to: String(new Date().getFullYear() + 50).slice(2),
    },
    MM: {
      mask: IMask.MaskedRange,
      from: 1,
...
function updateSecurityCode(code) {
  ...
  ccSecurity.innerText = code.length === 0 ? "212" : code
}
...
function uptadeExpirationDate(date) {
  ...
  ccExpiration.innerText = date.length === 0 ? "03/32" : date
}
